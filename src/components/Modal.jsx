...
const Modal = ({ isOpen, onClose, modalType }) => {
  ...
  const animationClass = isClosing
    ? isContact ? "animate-slide-out-left" : "animate-slide-out-right"
    : isContact ? "animate-slide-in-right" : "animate-slide-in-left";
  ...
  return (
    ...
    >
      <div
        className={`relative w-full max-w-xl bg-gray-900 text-white rounded-xl p-8 shadow-2xl min-h-[420px] flex ring-1 ring-indigo-400/40 shadow-[0_0_45px_rgba(96,165,250,0.35)] ${animationClass}`}
        ...
        {isContact ? (
          ...
          <div className="flex flex-col h-full w-full">
            ...
            <p className="text-base md:text-lg text-gray-200 leading-relaxed mb-4">
              ...
              focused on clean, responsive interfaces and production-ready React builds.
            </p>
            <p className="text-base md:text-lg text-gray-200 leading-relaxed mb-4">
            ...
            <div className="grid grid-cols-5 gap-4 justify-items-center mt-8 md:mt-10 pt-4 border-t border-gray-800 mt-auto">
              {techStack.map(({ icon, label, color }) => (
                <div key={label} className="flex flex-col items-center gap-1 p-2 bg-gray-800 rounded-lg">
